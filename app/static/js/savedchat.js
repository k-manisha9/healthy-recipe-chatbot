...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const response = await fetch(`/get_user_chats/${userId}`);
        const chats = await response.json();
        window.pineconeChats = chats;

        renderChatList(chats);

        if (chats.length > 0) {
            loadChat(chats[0].id);
            document.querySelectorAll('.chat-item')[0].classList.add('active');
        }
...
async function loadChat(chatId) {
    ...
    try {
        const res = await fetch(`/get_chat_history/${userId}/${chatId}`);
        ...
        history.history.forEach(([userMsg, botMsg]) => {
            ...
            if (botMsg) appendMessage('chatbot', botMsg);
        });
...
//     const savedChatsList = document.getElementById('saved-chats');
//     console.log("Rendering chat list:", savedChatsList);
...
function renderChatList(chats) {
    ...
    if (!Array.isArray(chats)) {
        ...
        savedChatsList.innerHTML = '<li class="error-item">No chats available</li>';
        return;
    }

    // Handle empty array
    if (chats.length === 0) {
        savedChatsList.innerHTML = '<li class="empty-item">No chats yet</li>';
        return;
    }

    // Process valid chats
    chats.forEach(chat => {
        if (!chat || !chat.id) return; // Skip invalid entries
...
async function deleteChat(chatId) {
    try {
        ...
        const res = await fetch(`/get_user_chats/${userId}`);
        if (!res.ok) {
        ...
        const chatWindow = document.getElementById('chat-window');
        chatWindow.innerHTML = '<div class="empty-state">Select a chat or start a new one</div>';
...
//     // const response = fetch(`/get_user_chats/${userId}`);
...
document.getElementById('new-chat-btn').addEventListener('click', async function () {
    const userId = localStorage.getItem('user_id');
    if (!userId) return alert("User ID not set");

    const newChatId = `${Date.now()}`;
    const chatTitle = `Chat ${newChatId}`;

    try {
        // Save chat metadata in Pinecone
        await fetch(`/register_chat/${userId}/${newChatId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
